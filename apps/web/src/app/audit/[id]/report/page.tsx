...
import { getOrgAudit } from "../../../(app)/audit/_actions/audit-actions";
...
export default async function ClientAuditReportPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const audit = await getOrgAudit(id);
  if (!audit) notFound();
          ...
          <tbody>
            ...
            <tr>
              ...
              <td style={{ borderBottom: "none" }}>
                {classLabels[audit.classification ?? "unknown"] ?? audit.classification}
              </td>
            </tr>
            <tr>
